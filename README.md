...
* **Algoritmo PPO:** Implementación con **Stable Baselines3** para el manejo de multiples entornos.
...
## Circuito y Auto
El circuito se encuentra en binario para poder manejar la lógica de fuera de pista. Se calculó una centerline a partír de un Signed Distance Field (SDF), parametrizada de forma continua para poder obtener el progreso en fomra de arco.

El auto cuenta con una velocidad máxima, tanto hacia adelante como en reversa. Al igual que en el juego, el auto está representado por un punto. Existe un drag constante y una fuerte resistencia fuera de pista y para la velocidad lateral (limitando el drift).
...
5.  **Penalización por Fuera de Pista (-):** Penalización por estar fuera de pista, y fin del intento y castigo si no regresa pasados 2 segundos.

En el segundo entrenamiento (optimización) se da mas libertad en la forma de manejar, pero se premia hacer de forma rápida la vuelta. Además se agregan los sectores. 
...
5. **Fuera de pista (-):** Mismo funcionamiento que en el primer entrenamiento. 
 
> ![Monitoreo del Entrenamiento](plots/T2.png) 

### Observaciones
El vector de estado que recibe el modelo incluye:
* Velocidad actual del vehículo, en la dirección frontal y en la perpendicular.
* Alineación con la centerline (El circuito).
* Dirección en la que se encuentra un punto futuro del circuito (calculado con la centerline), dado como coseno y seno del ángulo.
...
Monitoreo del `total_progress` (media entre los varios nucleos), valor del progreso acumulado de cada episodio, y `lap_time`. Un entrenamiento exitoso muestra una convergencia clara donde el agente logra completar varias vueltas de manera seguida y empieza a mejorar los tiempos de vuelta.
